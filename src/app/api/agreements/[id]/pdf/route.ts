/**
 * src/app/api/agreements/[id]/pdf/route.ts — Authenticated PDF download route.
 *
 * Requires authentication (office/owner roles only).
 * Fetches agreement data and generates a branded PDF via @react-pdf/renderer.
 *
...
export async function GET(
  ...
  try {
    ...
    // ── Build PDF props ───────────────────────────────────────────────────
    const pdfPoolEntries: AgreementPoolEntryPdfData[] = entryRows.map(
      (row) => ({
        poolId: row.pool_id,
    ...
    console.error("[GET /api/agreements/[id]/pdf] Error:", err)
